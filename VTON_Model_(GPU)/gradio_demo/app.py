...
print(f"✨ [AuraFit GPU Engine] Using device: {device}")
...
base_path = '../Model' if os.path.exists('../Model') else ('./Model' if os.path.exists('./Model') else os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'Model')))
example_path = os.path.join(os.path.dirname(__file__), 'example') if os.path.exists(os.path.join(os.path.dirname(__file__), 'example')) else os.path.join(os.path.dirname(__file__), '..', 'gradio_demo', 'example')

configs_path = os.path.join(os.path.dirname(__file__), '..', 'configs', 'densepose_rcnn_R_50_FPN_s1x.yaml') if not os.path.exists('./configs/densepose_rcnn_R_50_FPN_s1x.yaml') else './configs/densepose_rcnn_R_50_FPN_s1x.yaml'
ckpt_path = os.path.join(os.path.dirname(__file__), '..', 'ckpt', 'densepose', 'model_final_162be9.pkl') if not os.path.exists('./ckpt/densepose/model_final_162be9.pkl') else './ckpt/densepose/model_final_162be9.pkl'

print(f"📥 [AuraFit] Loading AI models from: {base_path}...")
...
print(f"📥 [AuraFit] Accelerating neural pipelines on {device}...")
pipe.to(device)
pipe.unet_encoder.to(device)
openpose_model.preprocessor.body_estimation.model.to(device)
print(f"✅ [AuraFit] AI Engine loaded and ready on {device}")

def start_tryon(dict, garm_img, garment_des, is_checked, is_checked_crop, denoise_steps, seed):
    """
    Main virtual try-on inference function
    """
    openpose_model.preprocessor.body_estimation.model.to(device)
    pipe.to(device)
    pipe.unet_encoder.to(device)
    
    print("🔄 [AuraFit] Processing virtual try-on request...")
    
    try:
        if garm_img is None:
            print("❌ [AuraFit] No garment image provided")
            return None, None
            
        garm_img = garm_img.convert("RGB").resize((768, 1024))
        
        if dict is None or "background" not in dict:
            print("❌ [AuraFit] No human image provided")
            return None, None
        ...
        if is_checked:
            ...
            if dict.get('layers') is None or len(dict['layers']) == 0:
                print("❌ [AuraFit] No mask layers provided")
                return None, None
        ...
        args = apply_net.create_argument_parser().parse_args(
            ('show', configs_path, ckpt_path, 'dp_segm', '-v', '--opts', 'MODEL.DEVICE', 'cpu')
        )
        pose_img = args.func(args, human_img_arg)
        pose_img = pose_img[:, :, ::-1]
        pose_img = Image.fromarray(pose_img).resize((768, 1024))
        
        print("🔄 [AuraFit] Generating photorealistic garment draping on GPU...")
        with torch.no_grad():
            with torch.cuda.amp.autocast() if 'cuda' in device else torch.no_grad():
                ...
                with torch.inference_mode():
                    ...
                    pose_img_tensor = tensor_transfrom(pose_img).unsqueeze(0).to(device, torch.float16 if 'cuda' in device else torch.float32)
                    garm_tensor = tensor_transfrom(garm_img).unsqueeze(0).to(device, torch.float16 if 'cuda' in device else torch.float32)
                    generator = torch.Generator(device).manual_seed(int(seed)) if seed is not None and seed >= 0 else None
                    
                    images = pipe(
                        prompt_embeds=prompt_embeds.to(device, torch.float16 if 'cuda' in device else torch.float32),
                        negative_prompt_embeds=negative_prompt_embeds.to(device, torch.float16 if 'cuda' in device else torch.float32),
                        pooled_prompt_embeds=pooled_prompt_embeds.to(device, torch.float16 if 'cuda' in device else torch.float32),
                        negative_pooled_prompt_embeds=negative_pooled_prompt_embeds.to(device, torch.float16 if 'cuda' in device else torch.float32),
                        num_inference_steps=int(denoise_steps),
                        generator=generator,
                        strength=1.0,
                        pose_img=pose_img_tensor,
                        text_embeds_cloth=prompt_embeds_c.to(device, torch.float16 if 'cuda' in device else torch.float32),
                        cloth=garm_tensor,
                        mask_image=mask,
                        image=human_img,
                        height=1024,
        ...
        print("✅ [AuraFit] Try-on completed successfully!")
        ...
        print(f"❌ [AuraFit] Error in try-on execution: {e}")
        import traceback
        traceback.print_exc()
        return None, None
